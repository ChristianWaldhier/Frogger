...
class Population:
    ...
    def crossover(self, brainA, brainB):
        ...
        for i in range(len(brainA.weights)):
            for j in range(len(brainA.weights[i])):
                for k in range(len(brainA.weights[i][j])):
    ...
    def mutation(self, brain):
        ...
        if prob <= mutationRate:
            for i in range(len(brain.weights)):
                for j in range(len(brain.weights[i])):
                    if random.randint(0, 1):
